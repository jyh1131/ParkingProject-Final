			...
			<div class="row align-items-center">
				...
				<button class="btn btn-primary" onclick="location.href ='./Menu'">메뉴</button>
					...
					<button type="button" class="btn btn-secondary" data-toggle="modal"
						data-target="#termsModal">자주 묻는 질문</button>
				</div>
			</div>
		</div>
	</header>
			...
			<c:if test="${not empty faqPage.content}">
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						<li class="page-item ${faqPage.first ? 'disabled' : ''}"><a
							class="page-link"
							href="?page=0&size=${faqPage.size}&searchQuery=${param.searchQuery}"
							aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
						</a></li>
						<c:forEach var="i"
							begin="${faqPage.number - 2 < 0 ? 0 : faqPage.number - 2}"
							end="${faqPage.number + 2 < faqPage.totalPages ? faqPage.number + 2 : faqPage.totalPages - 1}">
							<li class="page-item ${faqPage.number == i ? 'active' : ''}">
								<a class="page-link"
								href="?page=${i}&size=${faqPage.size}&searchQuery=${param.searchQuery}">${i + 1}</a>
							</li>
						</c:forEach>
						<li class="page-item ${faqPage.last ? 'disabled' : ''}"><a
							class="page-link"
							href="?page=${faqPage.totalPages - 1}&size=${faqPage.size}&searchQuery=${param.searchQuery}"
