...
<main>
	...
	<header id="main-header" class="py-2 btn-dark text-white">
		<div class="container">
			<div class="row align-items-center">
				...
				<button onclick="location.href ='./Menu'" class="btn btn-success btn-custom">메뉴</button>
					<c:if test="${member_rating == 'admin'}">
						<a href="./NoticeInsert" class="btn btn-success btn-custom">글
							작성</a>
					</c:if>
					<ul class="nav nav-tabs d-inline-flex">
						<li class="nav-item"><a class="nav-link active btn-custom"
							href="#">공지사항</a></li>
						<li class="nav-item"><a class="nav-link btn-custom"
							href="./NoticeSelectEvent">이벤트</a></li>
					</ul>
				</div>
			</div>
		</div>
	...
	<section id="notices">
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<form action="./NoticeSelect" method="get"
						...
						<div class="card-body">
								...
								<tbody>
									<c:forEach var="notice" items="${noticePage.content}">
										<tr class="text-center">
											...
											<td><a
												href="./NoticeSelectDetail?notice_code=${notice.notice_code}">${notice.notice_title}</a></td>
							...
							<c:choose>
								...
								<c:when test="${empty noticePage.content}">
									<div class="alert alert-warning" role="alert">등록된 공지사항이
										없습니다.</div>
								</c:when>
							</c:choose>
